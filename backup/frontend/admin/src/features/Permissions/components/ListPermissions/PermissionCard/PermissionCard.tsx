...
import { DeletePermission } from "../../../../../business/services";
import { useNavigate } from "react-router-dom";
interface Props {
  permission: IPermission;
  onRefresh?: () => void;
}
const PermissionCard = ({ permission, onRefresh }: Props) => {
  ...
  const confirmDelete = async () => {
    setIsDeleting(true);
    try {
      await DeletePermission({ PermissionID: permission._id as string });
      success("Permission Deleted", "The permission has been successfully moved to trash.");
      setIsDeleteModalOpen(false);
      if (onRefresh) {
        onRefresh();
      }
    } catch (err: unknown) {
      error("Error", (err as Error).message || "Failed to delete permission");
    } finally {
      setIsDeleting(false);
    }
  };
  const handleView = (e: React.MouseEvent) => {
    e.stopPropagation();
    navigate(`/permissions/${permission._id}`);
  };
    ...
    <>
      <div className="card h-100 shadow-sm border-0 bg-white" onClick={handleView} style={{ cursor: "pointer" }}>
        <div className="card-body d-flex align-items-center">
          ...
          >
            <span className="text-success fw-bold text-uppercase">{(permission as IPermission).Name?.substring(0, 1) || "C"}</span>
          </div>
          <div className="flex-grow-1 min-w-0">
            <h5 className="card-title mb-1 text-truncate">{(permission as IPermission).Name || "New Permission"}</h5>
            <p className="card-text text-muted small mb-0 text-truncate-2">
              {(permission as IPermission).Description || "No description provided."}
            </p>
          </div>
      ...
      <ConfirmModal
        ...
        message={`Are you sure you want to delete the permission "${(permission as IPermission).Name}"? This action will move it to the trash.`}
