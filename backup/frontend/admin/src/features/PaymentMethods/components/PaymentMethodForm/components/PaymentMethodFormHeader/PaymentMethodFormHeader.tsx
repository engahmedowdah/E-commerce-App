...
const PaymentMethodFormHeader: React.FC<Props> = ({ mode, title, loading, onCancel }) => {
    ...
    <div className={`flex justify-between ${isCreate ? 'items-start' : 'items-end pb-2'}`}>
      <div>
        <div className={`flex items-center gap-2 text-sm text-on-surface-variant ${isCreate ? 'mb-2' : 'mb-3'}`}>
          ...
          <span className="text-on-surface font-semibold">{isCreate ? 'Add New Payment Method' : 'Update Payment Method'}</span>
        </div>
        <h1 className={`${isCreate ? 'text-[40px]' : 'text-[38px]'} font-bold text-gray-900 mb-3 tracking-tight`}>
          {isCreate ? 'Create Payment Method' : (title || 'Update Payment Method')}
        </h1>
        <p className="text-[15px] text-gray-600 max-w-xl leading-relaxed">
          {isCreate
            ? 'Curate a new series of payment methods for the global marketplace. Defined by elegance and editorial precision.'
      ...
      <div className="flex items-center gap-3">
        ...
        >
          ...
          {isCreate ? 'Save Payment Method' : 'Update Payment Method'}
        </button>
      </div>
    </div>
  );
};
export default PaymentMethodFormHeader;
