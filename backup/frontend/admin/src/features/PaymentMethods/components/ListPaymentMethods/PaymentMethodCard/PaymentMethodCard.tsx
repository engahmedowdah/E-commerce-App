...
import { DeletePaymentMethod } from "../../../../../business/services";
import { useNavigate } from "react-router-dom";
interface Props {
  paymentmethod: IPaymentMethod;
  onRefresh?: () => void;
}
const PaymentMethodCard = ({ paymentmethod, onRefresh }: Props) => {
  ...
  const handleEdit = (e: React.MouseEvent) => {
    e.stopPropagation();
    setShowDropdown(false);
    navigate(`/payment-methods/edit/${paymentmethod._id}`);
  };
  ...
  const handleView = (e: React.MouseEvent) => {
    e.stopPropagation();
    navigate(`/payment-methods/${paymentmethod._id}`);
  };
    ...
    <>
      <div className="card h-100 shadow-sm border-0 bg-white" onClick={handleView} style={{ cursor: "pointer" }}>
        <div className="card-body d-flex align-items-center">
          ...
          >
            <span className="text-success fw-bold text-uppercase">{(paymentmethod as IPaymentMethod).Name?.substring(0, 1) || "C"}</span>
          </div>
          <div className="flex-grow-1 min-w-0">
            <h5 className="card-title mb-1 text-truncate">{(paymentmethod as IPaymentMethod).Name || "New PaymentMethod"}</h5>
          </div>
      ...
      <ConfirmModal
        ...
        message={`Are you sure you want to delete the payment method "${(paymentmethod as IPaymentMethod).Name}"? This action will move it to the trash.`}
