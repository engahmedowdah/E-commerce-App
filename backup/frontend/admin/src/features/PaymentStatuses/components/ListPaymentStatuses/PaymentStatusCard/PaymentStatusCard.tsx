...
import { DeletePaymentStatus } from "../../../../../business/services";
import { useNavigate } from "react-router-dom";
interface Props {
  paymentstatus: IPaymentStatus;
  onRefresh?: () => void;
}
const PaymentStatusCard = ({ paymentstatus, onRefresh }: Props) => {
  ...
  const handleEdit = (e: React.MouseEvent) => {
    e.stopPropagation();
    setShowDropdown(false);
    navigate(`/payment-statuses/edit/${paymentstatus._id}`);
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      success("Payment Status Deleted", "The payment status has been successfully deleted.");
  ...
  const handleView = (e: React.MouseEvent) => {
    e.stopPropagation();
    navigate(`/payment-statuses/${paymentstatus._id}`);
  };
    ...
    <>
      <div className="card h-100 shadow-sm border-0 bg-white" onClick={handleView} style={{ cursor: "pointer" }}>
        <div className="card-body d-flex align-items-center">
          ...
          >
            <span className="text-success fw-bold text-uppercase">{(paymentstatus as IPaymentStatus).Name?.substring(0, 1) || "C"}</span>
          </div>
          <div className="flex-grow-1 min-w-0">
            <h5 className="card-title mb-1 text-truncate">{(paymentstatus as IPaymentStatus).Name || "New Payment Status"}</h5>
          </div>
