...
import { DeleteCategory } from "../../../../../business/services";
import { useNavigate } from "react-router-dom";
interface Props {
  category: ICategory;
  onRefresh?: () => void;
}
const CategoryCard = ({ category, onRefresh }: Props) => {
    ...
    <>
      <div className="card h-100 shadow-sm border-0 bg-white" onClick={handleView} style={{ cursor: "pointer" }}>
        <div className="card-body d-flex align-items-center">
          ...
          >
            <span className="text-success fw-bold text-uppercase">{(category as ICategory).Name?.substring(0, 1) || "C"}</span>
          </div>
          <div className="flex-grow-1 min-w-0">
            <h5 className="card-title mb-1 text-truncate">{(category as ICategory).Name || "New Category"}</h5>
            <p className="card-text text-muted small mb-0 text-truncate-2">
              {(category as ICategory).Description || "No description provided."}
            </p>
          </div>
