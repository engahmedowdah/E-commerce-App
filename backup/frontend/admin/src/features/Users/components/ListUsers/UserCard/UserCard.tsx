...
import { DeleteUser } from "../../../../../business/services";
import { useNavigate } from "react-router-dom";
interface Props {
  user: IUser;
  onRefresh?: () => void;
}
const UserCard = ({ user, onRefresh }: Props) => {
    ...
    <>
      <div className="card h-100 shadow-sm border-0 bg-white" onClick={handleView} style={{ cursor: "pointer" }}>
        <div className="card-body d-flex align-items-center">
          ...
          >
            <span className="text-success fw-bold text-uppercase">{user.FirstName?.substring(0, 1) || "U"}</span>
          </div>
          <div className="flex-grow-1 min-w-0">
            <h5 className="card-title mb-1 text-truncate">{user.FirstName} {user.LastName}</h5>
