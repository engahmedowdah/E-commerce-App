...
const CollectionSEOPreview: React.FC<Props> = ({ mode, name, slug, description }) => {
  ...
  return (
    <div className="bg-white p-7 rounded-[20px] shadow-sm border border-gray-100">
      ...
      <div className="bg-[#F8FAFC] p-5 rounded-[16px] border border-blue-100/50">
        <div className="text-[17px] font-medium text-[#1E3A8A] mb-1.5 leading-snug">{name || "Summer Sale 2024"} | Premium Seasonal Collection</div>
        <div className="text-[13px] text-[#166534] mb-2.5 truncate font-medium">www.curator-store.com &rsaquo; collections &rsaquo; {slug || "summer-2024"}</div>
        <div className="text-[13px] text-[#475569] line-clamp-2 leading-relaxed">
          {description || "Discover the Summer Sale 2024 collection. High-end linen wear, expertly tailored to match modern lifestyle and ultimate comfort."}
        </div>
      </div>
    </div>
  );
};
export default CollectionSEOPreview;
