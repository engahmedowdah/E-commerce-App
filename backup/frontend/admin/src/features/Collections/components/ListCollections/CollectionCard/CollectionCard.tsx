...
import { DeleteCollection } from "../../../../../business/services";
import { useNavigate } from "react-router-dom";
interface Props {
  collection: ICollection;
  onRefresh?: () => void;
}
const CollectionCard = ({ collection, onRefresh }: Props) => {
  ...
  const handleEdit = (e: React.MouseEvent) => {
    e.stopPropagation();
    setShowDropdown(false);
    navigate(`/collections/edit/${collection._id}`);
  };
    ...
    <>
      <div className="card h-100 shadow-sm border-0 bg-white" onClick={handleView} style={{ cursor: "pointer" }}>
        <div className="card-body d-flex align-items-center">
          ...
          >
            <span className="text-success fw-bold text-uppercase">{(collection as ICollection).Name?.substring(0, 1) || "C"}</span>
          </div>
          <div className="flex-grow-1 min-w-0">
            <h5 className="card-title mb-1 text-truncate">{(collection as ICollection).Name || "New Collection"}</h5>
            <p className="card-text text-muted small mb-0 text-truncate-2">
              {(collection as ICollection).Description || "No description provided."}
            </p>
          </div>
