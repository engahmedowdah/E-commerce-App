...
const OrderStatusFormHeader: React.FC<Props> = ({ mode, title, loading, onCancel }) => {
    ...
    <div className={`flex justify-between ${isCreate ? 'items-start' : 'items-end pb-2'}`}>
      <div>
        <div className={`flex items-center gap-2 text-sm text-on-surface-variant ${isCreate ? 'mb-2' : 'mb-3'}`}>
          <Link to="/order-statuses" className="flex items-center gap-1 hover:text-primary transition-colors hover-translate-y">
            ...
            <span className="font-medium">Order Statuses</span>
          </Link>
          <span className="material-symbols-outlined text-[18px]">chevron_right</span>
          <span className="text-on-surface font-semibold">{isCreate ? 'Add New Order Status' : 'Update Order Status'}</span>
        </div>
        <h1 className={`${isCreate ? 'text-[40px]' : 'text-[38px]'} font-bold text-gray-900 mb-3 tracking-tight`}>
          {isCreate ? 'Create Order Status' : (title || 'Update Order Status')}
        </h1>
        <p className="text-[15px] text-gray-600 max-w-xl leading-relaxed">
          {isCreate
            ...
            : 'Update order status details, hero imagery, and associated products.'}
        </p>
      </div>
      <div className="flex items-center gap-3">
        ...
        >
          ...
          {isCreate ? 'Save Order Status' : 'Update Order Status'}
        </button>
      </div>
    </div>
  );
};
export default OrderStatusFormHeader;
