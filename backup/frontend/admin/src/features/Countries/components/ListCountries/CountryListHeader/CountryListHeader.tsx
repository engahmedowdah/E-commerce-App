...
const CountryListHeader = ({ currentSort, onSortChange }: { currentSort: "newest" | "oldest" | "name_asc" | "name_desc" | string; onSortChange: (sort: string) => void }) => {
  ...
  const sortOptions = [
    ...
    { value: "name_desc", label: t.sort?.nameDesc || "Name Z-A" },
  ];
  ...
  const handleSortSelect = (sort: string) => {
    ...
    return (
        <div className="px-6 py-4 border-b border-surface-container flex items-center justify-between">
            <h3 className="text-lg font-bold text-primary">{(t.list as any).allCountries}</h3>
