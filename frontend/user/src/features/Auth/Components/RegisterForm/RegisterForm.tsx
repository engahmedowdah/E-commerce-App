...
function RegisterForm(): React.ReactNode {
  const [showPassword, setShowPassword] = useState(false);

  return (
    <section className="w-full md:w-1/2 p-8 md:p-16 flex flex-col justify-center">
      <div className="mb-10 text-center md:text-right">
        <h2 className="font-headline-lg text-headline-lg text-on-surface mb-2">إنشاء حساب جديد</h2>
        <p className="font-body-sm text-body-sm text-on-surface-variant">الرجاء إدخال تفاصيلك لإنشاء حساب</p>
      </div>

      <form className="space-y-6" onSubmit={(e) => e.preventDefault()}>
        <div>
          <label className="block font-label-bold text-label-bold text-on-surface mb-2" htmlFor="name">الاسم الكامل</label>
          <div className="relative">
            <span className="material-symbols-outlined absolute right-4 top-1/2 -translate-y-1/2 text-on-surface-variant">person</span>
            <input className="w-full px-12 py-4 bg-surface-container-low border-none rounded-xl font-body-lg text-body-lg focus:ring-2 focus:ring-primary-container transition-all" id="name" placeholder="أدخل اسمك الكامل" type="text" />
          </div>
        </div>
        ...
        <div>
          <label className="block font-label-bold text-label-bold text-on-surface mb-2" htmlFor="password">كلمة المرور</label>
          <div className="relative">
        ...
        <div className="flex items-start gap-3">
          ...
          <label className="font-body-sm text-body-sm text-on-surface-variant cursor-pointer leading-tight" htmlFor="terms">
            أوافق على <a className="text-primary hover:underline" href="#">الشروط والأحكام</a> و <a className="text-primary hover:underline" href="#">سياسة الخصوصية</a>
          </label>
        </div>

        <button className="w-full bg-black text-white font-label-bold text-label-bold py-4 rounded-xl hover:bg-gray-900 active:scale-[0.98] transition-all flex items-center justify-center gap-2 cursor-pointer border-none" type="submit">
      ...
      <div className="mt-8 text-center">
        <p className="font-body-sm text-body-sm text-on-surface-variant">
          لديك حساب بالفعل؟ 
          <Link className="text-primary font-label-bold text-label-bold hover:underline mr-1" to="/auth/login">تسجيل الدخول</Link>
        </p>
      </div>
    </section>
  );
