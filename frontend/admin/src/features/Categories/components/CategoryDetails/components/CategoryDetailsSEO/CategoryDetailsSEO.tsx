...
const CategoryDetailsSEO: React.FC<Props> = ({ category }) => {
  return (
    <div className="bg-surface-container-lowest rounded-3xl p-8 border border-outline-variant/30 shadow-sm glass-panel hover:shadow-md transition-shadow">
      ...
      <div className="space-y-6">
         <div className="pb-4 border-b border-outline-variant/20">
            ...
            <p className="text-sm font-medium text-on-surface bg-surface-container-low/50 px-3 py-2 rounded-lg font-mono text-primary/90 select-all border border-outline-variant/20">/{(category as ICategory).Slug}</p>
         </div>
         <div className="pb-4 border-b border-outline-variant/20">
            <p className="text-xs font-semibold text-on-surface-variant uppercase tracking-wider mb-2">Meta Title</p>
            <p className="text-base font-medium text-on-surface">{(category as ICategory).Name} - Category</p>
         </div>
         <div>
            <p className="text-xs font-semibold text-on-surface-variant uppercase tracking-wider mb-2">Meta Description</p>
            <p className="text-sm text-on-surface-variant leading-relaxed">
                {(category as ICategory).Description?.substring(0, 150) || 'No meta description set for this category. Add a description to improve SEO.'}
            </p>
         </div>
      </div>
    </div>
...
export default CategoryDetailsSEO;
