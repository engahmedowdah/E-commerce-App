...
const EditPaymentStatusForm: React.FC<Props> = ({
  formData,
  loading,
  onChange,
  onSubmit,
  ...
  return (
    <>
      <form onSubmit={onSubmit} className="flex flex-col gap-8 w-full max-w-6xl mx-auto">
        <PaymentStatusFormHeader
          mode="edit"
          title={(formData as unknown as IPaymentStatus).Name}
          loading={loading}
          onCancel={onCancel}
        />
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
          ...
          <div className="md:col-span-4 flex flex-col gap-6">
            <PaymentStatusVisibility
              mode="edit"
              isActivated={(formData as unknown as IPaymentStatus).IsActivated}
              onChange={onChange}
            />
