...
const PaymentStatusGeneralInfo: React.FC<Props> = ({ mode, formData, onChange }) => {
  ...
  return (
    <div className="bg-white p-8 rounded-[20px] shadow-sm border border-gray-100">
      <h3 className={`${isCreate ? 'text-[13px] tracking-widest uppercase' : 'text-[17px]'} font-bold text-gray-900 mb-8`}>
        {isCreate ? 'General Information' : 'PaymentStatus Details'}
      </h3>
      <div className="mb-6">
        <label className={`block ${isCreate ? 'text-[14px] text-gray-900' : 'text-[12px] text-gray-500 tracking-widest uppercase'} font-bold mb-2`}>
          {isCreate ? 'PaymentStatus Name' : 'PaymentStatus Title'}
        </label>
        <input
          type="text"
          name="Name"
          value={(formData as unknown as IPaymentStatus).Name}
          onChange={onChange}
...
export default PaymentStatusGeneralInfo;
