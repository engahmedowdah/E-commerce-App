...
import { DeleteAdmin } from "../../../../../business/services";
import { useNavigate } from "react-router-dom";
interface Props {
  admin: IAdmin;
  onRefresh?: () => void;
}
const AdminCard = ({ admin, onRefresh }: Props) => {
    ...
    <>
      <div className="card h-100 shadow-sm border-0 bg-white" onClick={handleView} style={{ cursor: "pointer" }}>
        <div className="card-body d-flex align-items-center">
          ...
          >
            <span className="text-success fw-bold text-uppercase">{(admin as IAdmin).FirstName?.substring(0, 1) || "C"}</span>
          </div>
          <div className="flex-grow-1 min-w-0">
            <h5 className="card-title mb-1 text-truncate">{(admin as IAdmin).FirstName + " " + (admin as IAdmin).LastName || "New Admin"}</h5>
            <p className="card-text text-muted small mb-0 text-truncate-2">
              {(admin as IAdmin).Email || "No email provided."}
            </p>
          </div>
