...
const AdminFormHeader: React.FC<Props> = ({ mode, title, loading, onCancel }) => {
    ...
    <div className={`flex justify-between ${isCreate ? 'items-start' : 'items-end pb-2'}`}>
      <div>
        <div className={`flex items-center gap-2 text-sm text-on-surface-variant ${isCreate ? 'mb-2' : 'mb-3'}`}>
          ...
          <span className="text-on-surface font-semibold">{isCreate ? 'Add New Admin' : 'Update Admin'}</span>
        </div>
        <h1 className={`${isCreate ? 'text-[40px]' : 'text-[38px]'} font-bold text-gray-900 mb-3 tracking-tight`}>
          {isCreate ? 'Create Admin' : (title || 'Update Admin')}
        </h1>
        <p className="text-[15px] text-gray-600 max-w-xl leading-relaxed">
          {isCreate
            ...
            : 'Update admin details, hero imagery, and associated products.'}
        </p>
      </div>
      <div className="flex items-center gap-3">
        ...
        >
          ...
          {isCreate ? 'Save Admin' : 'Update Admin'}
        </button>
      </div>
    </div>
  );
