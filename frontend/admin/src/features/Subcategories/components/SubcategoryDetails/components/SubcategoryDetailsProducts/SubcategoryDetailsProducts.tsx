...
const SubcategoryDetailsProducts: React.FC<Props> = ({ products }) => {
  return (
    <div className={`bg-surface-container-lowest rounded-3xl p-8 md:p-12 border border-outline-variant/30 shadow-sm glass-panel flex flex-col w-full hover:shadow-md transition-shadow mb-12 ${products.length === 0 ? 'justify-center items-center min-h-[24rem]' : 'justify-start items-stretch min-h-[24rem]'}`}>
      {products.length === 0 ? (
        <>
          ...
          <p className="text-on-surface-variant mb-6 text-center max-w-md">Manage products assigned to this subcategory. You can add new products or remove existing ones.</p>
              ...
              <div key={product._id} className="bg-surface-container-low rounded-2xl overflow-hidden border border-outline-variant/30 shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300 group flex flex-col">
                <div className="w-full h-48 bg-surface-container relative overflow-hidden">
                  ...
                  <div className="absolute top-3 right-3 flex flex-col gap-2 items-end">
                    <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider shadow-sm backdrop-blur-md ${(product as any).IsActivated ? 'bg-green-500/90 text-white' : 'bg-surface-container-highest/90 text-on-surface'}`}>
                      {(product as any).IsActivated ? 'Active' : 'Draft'}
                    </span>
                ...
                <div className="p-5 flex flex-col flex-1">
                  <div className="mb-2">
                    <h4 className="text-lg font-bold text-on-surface mb-1 truncate group-hover:text-primary transition-colors">{(product as any).Name}</h4>
                    <p className="text-sm text-on-surface-variant line-clamp-2 min-h-[2.5rem]">
                      {(product as any).Description || 'No description provided.'}
                    </p>
                  </div>
...
export default SubcategoryDetailsProducts;
