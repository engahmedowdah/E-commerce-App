...
const OrderDetailsProducts: React.FC<Props> = ({ products, currencySymbol = '$' }) => {
  return (
    <div className={`bg-surface-container-lowest rounded-3xl p-8 md:p-12 border border-outline-variant/30 shadow-sm glass-panel flex flex-col w-full hover:shadow-md transition-shadow mb-12 ${products.length === 0 ? 'justify-center items-center min-h-[24rem]' : 'justify-start items-stretch min-h-[24rem]'}`}>
      {products.length === 0 ? (
        <>
          ...
          <h3 className="text-2xl font-bold text-on-surface mb-3 tracking-tight">Products</h3>
          <p className="text-on-surface-variant mb-6 text-center max-w-md">No products assigned to this order.</p>
        </>
      ) : (
        <>
          ...
          <div className="w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 animate-fade-in">
            {products.map((product) => (
              <Link to={`/products/details/${(product as IOrderProduct).Product._id}`} key={product.ProductID} className="bg-surface-container-low rounded-2xl overflow-hidden border border-outline-variant/30 shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300 group flex flex-col">
                <div className="w-full h-48 bg-surface-container relative overflow-hidden">
                  {product.Product.Images && product.Product.Images.length > 0 ? (
                    <img
                      ...
                      alt={(product as IOrderProduct).Product.Name}
                  ...
                  <div className="absolute top-3 right-3 flex flex-col gap-2 items-end">
                    <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider shadow-sm backdrop-blur-md ${(product as IOrderProduct).Product.IsActivated ? 'bg-green-500/90 text-white' : 'bg-surface-container-highest/90 text-on-surface'}`}>
                      {(product as IOrderProduct).Product.IsActivated ? 'Active' : 'Draft'}
                    </span>
                    <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider shadow-sm backdrop-blur-md ${product.Quantity > 0 ? 'bg-primary/90 text-white' : 'bg-error/90 text-white'}`}>
                ...
                <div className="p-5 flex flex-col flex-1">
                  <div className="mb-2">
                    <h4 className="text-lg font-bold text-on-surface mb-1 truncate group-hover:text-primary transition-colors">{(product as IOrderProduct).Product.Name}</h4>
                    <p className="text-sm text-on-surface-variant line-clamp-2 min-h-[2.5rem]">
                      {(product as IOrderProduct).Product.Description || 'No description provided.'}
                    </p>
                  </div>
                  <div className="flex items-center justify-between mt-auto pt-4 border-t border-outline-variant/20">
                    <span className="text-xl font-black text-on-surface">{currencySymbol}{product.Product.Price.toFixed(2)}</span>
                    <div className="flex gap-2">
                       <Link
                         to={`/products/edit/${(product as IOrderProduct).Product._id}`}
