    ...
    <>
      <div className="card h-100 shadow-sm border-0 bg-white" onClick={handleView} style={{ cursor: "pointer" }}>
        <div className="card-body d-flex align-items-center">
          ...
          >
            <span className="text-success fw-bold text-uppercase">{order.User?.FirstName + " " + order.User?.LastName}</span>
          </div>
          <div className="flex-grow-1 min-w-0">
            <h5 className="card-title mb-1 text-truncate">{(order as IOrder).User?.FirstName + " " + (order as IOrder).User?.LastName}</h5>
            <p className="card-text text-muted small mb-0 text-truncate-2">
              {(order as IOrder).User?.Email}
            </p>
            <span className="badge bg-primary">{order.OrderStatus.Name}</span>
            <p>Order Date: {new Date((order as IOrder).CreatedDate).toLocaleDateString()}</p>
            <p>Total Amount: {order.Payment.Amount} {order.Payment.CurrencySymbol}</p>
          </div>
