...
const CollectionGeneralInfo: React.FC<Props> = ({ mode, formData, onChange, onGenerateSlug }) => {
  ...
  return (
    <div className="bg-white p-8 rounded-[20px] shadow-sm border border-gray-100">
      <h3 className={`${isCreate ? 'text-[13px] tracking-widest uppercase' : 'text-[17px]'} font-bold text-gray-900 mb-8`}>
        {isCreate ? 'General Information' : 'Collection Details'}
      </h3>
      <div className="mb-6">
        <label className={`block ${isCreate ? 'text-[14px] text-gray-900' : 'text-[12px] text-gray-500 tracking-widest uppercase'} font-bold mb-2`}>
          {isCreate ? 'Collection Name' : 'Collection Title'}
        </label>
        <input
          type="text"
          name="Name"
          value={(formData as unknown as ICollection).Name}
          onChange={onChange}
      ...
      <div className="mb-6">
        ...
        <div className="flex gap-3">
          <input
            type="text"
            name="Slug"
            value={(formData as unknown as ICollection).Slug}
            onChange={onChange}
            className="w-full bg-[#F3F4F6] border-none rounded-[12px] px-5 py-3.5 text-[15px] focus:bg-gray-200/50 focus:outline-none focus:ring-2 focus:ring-gray-900/10 transition-colors placeholder:text-gray-400"
            placeholder={isCreate ? "e.g. winter-solstice-2024" : "summer-sale-2024"}
          />
      ...
      <div className={isCreate ? 'mb-8' : ''}>
        ...
        <textarea
          name="Description"
          value={(formData as unknown as ICollection).Description}
          onChange={onChange}
          rows={isCreate ? 6 : 5}
          ...
          placeholder={isCreate ? "Describe the soul and story of this collection..." : "Our annual curated selection of seasonal essentials..."}
        ></textarea>
      </div>
    </div>
  );
