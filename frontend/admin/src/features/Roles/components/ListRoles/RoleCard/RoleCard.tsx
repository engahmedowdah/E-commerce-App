...
import { DeleteRole } from "../../../../../business/services";
import { useNavigate } from "react-router-dom";
interface Props {
  role: IRole;
  onRefresh?: () => void;
}
const RoleCard = ({ role, onRefresh }: Props) => {
  ...
  const confirmDelete = async () => {
    setIsDeleting(true);
    try {
      await DeleteRole({ RoleID: role._id as string });
      success("Role Deleted", "The role has been successfully moved to trash.");
      setIsDeleteModalOpen(false);
      if (onRefresh) {
        onRefresh();
      }
    } catch (err: unknown) {
      error("Error", (err as Error).message || "Failed to delete role");
    } finally {
      setIsDeleting(false);
    }
  };
    ...
    <>
      <div className="card h-100 shadow-sm border-0 bg-white" onClick={handleView} style={{ cursor: "pointer" }}>
        <div className="card-body d-flex align-items-center">
          ...
          >
            <span className="text-success fw-bold text-uppercase">{(role as IRole).Name?.substring(0, 1) || "C"}</span>
          </div>
          <div className="flex-grow-1 min-w-0">
            <h5 className="card-title mb-1 text-truncate">{(role as IRole).Name || "New Role"}</h5>
            <p className="card-text text-muted small mb-0 text-truncate-2">
              {(role as IRole).Description || "No description provided."}
            </p>
          </div>
      ...
      <ConfirmModal
        ...
        title="Delete Role"
        message={`Are you sure you want to delete the role "${(role as IRole).Name}"? This action will move it to the trash.`}
